...
function createServer() {
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify(
              { by_status: byStatus, last_14_days: recent },
              null,
              2,
            ),
          },
  ...
  server.tool(
    ...
    'List benefit applications with optional status filter and search by applicant name',
    {
      ...
      search: z
        .string()
        .optional()
        .describe('Search by applicant first or last name'),
    ...
    async ({ status, search, limit, offset }) => {
      ...
      if (search) {
        params.push(`%${search}%`);
        where.push(
          `(ap.first_name ILIKE $${params.length} OR ap.last_name ILIKE $${params.length})`,
        );
      }
  ...
  server.tool(
    ...
    async ({ application_id }) => {
      const [
        ...
        pool.query(
          `SELECT * FROM public.household_members WHERE application_id = $1::uuid ORDER BY created_at`,
          [application_id],
        ),
        pool.query(
          `SELECT * FROM public.incomes WHERE application_id = $1::uuid ORDER BY created_at`,
          [application_id],
        ),
        pool.query(
          `SELECT id, document_type, file_url, mime_type, uploaded_at FROM public.documents WHERE application_id = $1::uuid ORDER BY uploaded_at`,
          [application_id],
        ),
        pool.query(
          `SELECT rule_name, severity, message, resolved FROM public.validation_results WHERE application_id = $1::uuid ORDER BY severity`,
          [application_id],
        ),
        pool.query(
          `SELECT estimated_program, fpl_percentage, screening_result, created_at FROM public.eligibility_screenings WHERE application_id = $1::uuid ORDER BY created_at DESC LIMIT 1`,
          [application_id],
        ),
        pool.query(
          `SELECT message, due_date, resolved, created_at FROM public.rfis WHERE application_id = $1::uuid ORDER BY created_at DESC`,
          [application_id],
        ),
      ]);
  ...
  server.tool(
    ...
    {
      social_worker_user_id: z
        .string()
        .uuid()
        .describe('Social worker user UUID'),
    },
  ...
  server.tool(
    ...
    'Search for users (applicants, social workers, reviewers) by name or email',
    {
      ...
      role: z
        .enum(['applicant', 'social_worker', 'reviewer', 'admin'])
        .optional(),
    },
    async ({ query, role }) => {
      ...
      if (role) {
        params.push(role);
        roleJoin = `AND EXISTS (SELECT 1 FROM public.user_roles ur JOIN public.roles ro ON ro.id = ur.role_id WHERE ur.user_id = u.id AND ro.name = $${params.length})`;
      }
  ...
  server.tool(
    ...
    'All unresolved Requests for Information across applications, optionally filtered by due date urgency',
    {
      overdue_only: z
        .boolean()
        .default(false)
        .describe('Only show RFIs past their due date'),
    },
    async ({ overdue_only }) => {
      const { rows } = await pool.query(`
  ...
  server.tool(
    ...
    'Recent audit log entries, optionally scoped to a specific application',
    {
      application_id: z
        ...
        .uuid()
        .optional()
        .describe('Scope to one application'),
