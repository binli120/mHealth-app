...
if container_running "healthcompass-proxy"; then
  ...
  # 404 is OK — Traefik is up but 'localhost' doesn't match any router rule
  # (all routers are keyed on your domain). Any non-000 response means TLS works.
...
if [[ "$FAILURES" -eq 0 && "$WARNINGS" -eq 0 ]]; then
  ...
  echo -e "  ${RED}${BOLD}$FAILURES failure(s), $WARNINGS warning(s). Attempting restart…${NC}"
  echo ""

  if [[ ! -d "$APP_DIR" ]]; then
    echo -e "  ${RED}ERROR: App directory $APP_DIR not found — cannot restart.${NC}"
    exit 1
  fi

  echo -e "  Running: cd $APP_DIR && docker compose --env-file $ENV_FILE down && docker compose --env-file $ENV_FILE up -d"
  cd "$APP_DIR"
  docker compose --env-file "$ENV_FILE" down
  docker compose --env-file "$ENV_FILE" up -d
  echo ""
  echo -e "  Waiting 15s for containers to start…"
  sleep 15

  # Re-run the same script to verify — but without auto-restart to avoid loops
  echo -e "${BOLD}── Re-checking after restart ──${NC}"
  FAILURES=0
  WARNINGS=0
  ...
  for name in "${CONTAINER_ORDER[@]}"; do
    ...
    if container_running "$name"; then
      ...
      fail "$label ($name)" "still down after restart"
    fi
  done

  echo ""
  if [[ "$FAILURES" -eq 0 ]]; then
    echo -e "  ${GREEN}${BOLD}All services recovered successfully.${NC}"
  else
    echo -e "  ${RED}${BOLD}$FAILURES service(s) still down after restart. Manual intervention required.${NC}"
