...
function displayPath(filePath) {
  if (!filePath) return "unknown test file"

  const cwdRelative = relative(process.cwd(), filePath)
  if (cwdRelative && !cwdRelative.startsWith("..")) return cwdRelative
...
for (const file of results.testResults ?? []) {
  ...
  if (failedTests.length > 0) {
    failedSuites.push({ file: suiteFilePath(file), tests: failedTests })
  }
}
...
for (const suite of failedSuites) {
  const shortFile = displayPath(suite.file)
  ...
  await createIssue({
    ...
    body: [
      ...
      "### Reproduce Locally",
      "```bash",
      suite.file ? `pnpm test ${shortFile}` : "pnpm test",
      "```",
      "",
